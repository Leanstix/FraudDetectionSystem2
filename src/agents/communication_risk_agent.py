...
class CommunicationRiskAgent(BaseAgent):
    ...
    def score_threads(self, sms_df: pd.DataFrame, mails_df: pd.DataFrame, dataset_name: str) -> dict:
        ...
        for thread_id, grp in sms_df.groupby("thread_id"):
            text = "\n".join(grp["message_text"].astype(str).tolist())
            scored_sms[thread_id] = self.analyzer.analyze_sms_thread(text, dataset_name=dataset_name)

        for thread_id, grp in mails_df.groupby("thread_id"):
            text = (grp["subject"].fillna("") + "\n" + grp["body_text"].fillna("")).str.cat(sep="\n")
            scored_mails[thread_id] = self.analyzer.analyze_mail_thread(text, dataset_name=dataset_name)
    ...
    def run(self, features_df: pd.DataFrame, sms_df: pd.DataFrame, mails_df: pd.DataFrame, dataset_name: str) -> pd.DataFrame:
        ...
        events: list[dict] = []
        for thread_id, grp in sms_df.groupby("thread_id"):
            score_payload = thread_scores["sms"].get(thread_id, {})
            ts = grp["timestamp"].dropna().min()
            participant_text = " ".join(grp[["sender", "recipient"]].fillna("").astype(str).values.ravel().tolist()).lower()
            events.append(
                {
                    "timestamp": ts,
        ...
        for thread_id, grp in mails_df.groupby("thread_id"):
            score_payload = thread_scores["mails"].get(thread_id, {})
            ts = grp["timestamp"].dropna().min()
            participant_text = " ".join(grp[["sender", "recipient", "subject"]].fillna("").astype(str).values.ravel().tolist()).lower()
            events.append(
                {
                    "timestamp": ts,
                    "participants": participant_text,
        ...
        for idx, row in out.iterrows():
            ...
            decayed = scoped["risk_score"].values * np.exp(-hours.values / 72.0)
